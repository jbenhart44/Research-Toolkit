...
4. **The File Manifest is the integrity check**: Each member writes a separate `member_{seat}.md` file (>500 bytes, unique SHA-256) plus `chair_synthesis.md` in `.toolkit/evidence/coa_sessions/coa_{date}_{slug}/`; a single `all_members.md` is rejected as single-context fallback.
...
- **INVESTIGATE: Whether `scripts/audit_run_evidence.sh` and a council-sessions directory exist in your project** before first use. The self-audit in Step 5b will fail loudly if they don't; either run the command once to let it create the directory, or `mkdir -p .toolkit/evidence/coa_sessions/` manually. Effort: 1 minute.
...
3. **The PACE-vs-CoA comparison table pattern is the missing piece in most toolkit commands.** All three lenses independently landed on this. Practical next step: audit each of the 14 command specs for a "vs. the obvious alternative" table. Commands that lack one (likely most of them) need it added before they're ready for the per-command-docs pass.

### Suggested next command
- For shipping /coa's website description right now: just edit the Editor's 3-paragraph structure into a new file and link it from your toolkit website. No further command needed.
